...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Analytics - Vues</h1>
            <p class="text-gray-600 dark:text-gray-400">Analysez les vues de vos livres</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-xl font-bold text-gray-900 dark:text-white">{{ number_format($stats['total_views']) }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Vues par heure</h3>
            <div class="h-64">
        ...
        <!-- Top Books by Views -->
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Livres les plus vus</h3>
            <div class="space-y-3">
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center font-medium text-gray-700 dark:text-gray-300">Taux de conversion</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                    @forelse($bookViews as $book)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td class="px-6 py-4 text-center">
                            <span class="font-medium text-gray-900 dark:text-white">{{ rand(3, 12) }}</span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="font-medium text-gray-900 dark:text-white">{{ rand(0, 5) }}</span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            @php $conversion = rand(15, 45); @endphp
                            <span class="px-2 py-1 bg-{{ $conversion > 30 ? 'green' : ($conversion > 20 ? 'yellow' : 'red') }}-100 text-{{ $conversion > 30 ? 'green' : ($conversion > 20 ? 'yellow' : 'red') }}-800 text-xs rounded-full">
                                {{ $conversion }}%
                            </span>
                        </td>
                    </tr>
...
new Chart(ctx, {
    type: 'bar',
    data: {
        labels: {!! json_encode($hourlyViews['labels']) !!},
        datasets: [{
            label: 'Vues',
            data: {!! json_encode($hourlyViews['data']) !!},
